...
class S2TJoint(BaseFairseqModel):
    ...
    def __init__(self, acoustic_encoder, textual_encoder, decoder):
        super().__init__()
    ...
    def build_decoder(cls, cfg, task, embed_tokens):
        ...
        #if getattr(cfg, "mt_model_path", None):
        #    logger.info(
        #        f"load pretrained decoder from: "
        #        f"{cfg.mt_model_path}"
        #    )
        #    decoder = checkpoint_utils.load_pretrained_component_from_model(
        #        component=decoder, checkpoint=cfg.mt_model_path, strict=False
        #    )
    ...
    def build_model(cls, args, task):
        ...
        return cls(acoustic_encoder, textual_encoder, decoder)
...
class AcousticEncoder(Wav2vecEncoder):
    ...
    def forward(self, src_tokens, src_lengths, tbc=True, **kwargs):
