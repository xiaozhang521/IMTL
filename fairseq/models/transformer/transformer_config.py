...
class EncDecBaseConfig(FairseqDataclass):
    ...
    embed_conv_kernel: int = field(
        default=0,
        metadata={
            "help": "use the conv on word embedings"
        },
    )
    encoder_conv: int = field(
        default=0,
        metadata={
...
class TransformerConfig(FairseqDataclass):
    ...
    max_relative_length: int =  field(
        default=-1,
        metadata={"help":"the max relative length"},
    )

    k_only: bool = field(
        default=False,
        metadata={"help":"select the relative mode to map relative position information"},
    )
