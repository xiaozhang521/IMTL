...
class TransformerEncoderBase(FairseqEncoder):
    ...
    def __init__(self, cfg, dictionary, embed_tokens, return_fc=False):
        ...
        if cfg.encoder.embed_conv_kernel and  cfg.encoder.encoder_conv == 0:
            self.conv_norm = LayerNorm(embed_dim)
            activation_fn = utils.get_activation_fn(
                activation='relu'
            )
            self.conv_module = ConvolutionModule(embed_dim, cfg.encoder.embed_conv_kernel, activation_fn)
        else:
            self.conv_module = None
        if cfg.encoder.encoder_conv > 0:
            self.conv_norm_list = nn.ModuleList([])
            self.conv_norm_list.extend(
                [LayerNorm(embed_dim) for i in range(cfg.encoder.encoder_conv)]
            )
    ...
    def forward_embedding(
        ...
        token_embedding = self.embed_scale * token_embedding
        if self.conv_module is not None:
            encoder_padding_mask = src_tokens.eq(self.padding_idx)
            residual = token_embedding
            token_embedding = self.conv_norm(token_embedding)
            token_embedding = residual + self.conv_module(token_embedding, encoder_padding_mask)
    ...
    def forward_scriptable(
        ...
        #    residual = x
        #    x = self.conv_norm(x)
        #    x = residual + self.conv_module(x, encoder_padding_mask)
        ...
        for index, layer in enumerate(self.layers):
            ...
            if self.conv_list and index < len(self.conv_list):
                ...
                x = self.conv_list[index](x.transpose(0, 1), encoder_padding_mask)
                x = x.transpose(0, 1)
                x = residual + x 
