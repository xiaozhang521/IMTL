    ...
    def _per_task_train_loss(
        ...
        if ignore_grad:
            ...
            if per_task == "mt":
                ...
                loss *= self.asr_weight
        optimizer.backward(loss)
        norm_list = []
        if update_num % 5000 == 0 and update_num > 3000:
            #print(per_task)
            if per_task != "mt":
                tmp_list=[]
    ...
    def train_step(
        ...
        for idx, per_task in enumerate(sample.keys()): #enumerate(["st","asr","mt"]): #
            ...
            if per_task == "st":
                ...
                mt_list = norm_list
            #if per_task == "st" and update_num % 2000 == 0 :
            #    for index in range(6):
            #        grad_test2_a=model.acoustic_encoder.sead.layers[index].fc1.weight
            #        grad_test2_b=model.acoustic_encoder.sead.layers[index].self_attn.out_proj.weight
            #        mt_grad_a_list.append(grad_test2_a.grad)
            #        mt_grad_b_list.append(grad_test2_b.grad)
            #if per_task == "mt" and update_num % 2000 == 0 :
            #    for index in range(6):
            #        grad_test2_a=model.acoustic_encoder.sead.layers[index].fc1.weight
            #        grad_test2_b=model.acoustic_encoder.sead.layers[index].self_attn.out_proj.weight
            #        grad2_a=F.cosine_similarity(grad_test2_a.grad.flatten(), mt_grad_a_list[index].flatten(), dim=0).detach().cpu().tolist()
            #        grad2_b=F.cosine_similarity(grad_test2_b.grad.flatten(), mt_grad_b_list[index].flatten(), dim=0).detach().cpu().tolist()
            #        print("similarity", grad2_a, grad2_b)
            #    print()
